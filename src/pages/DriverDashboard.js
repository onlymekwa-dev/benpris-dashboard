...
import {
  AppLayout, PageHeader, KpiRow, StatCard, ChartCard,
  Table, C, fmt, Spinner, useIsMobile,
} from '../components/UI';

// Find driver record by profile_id → full_name → email, auto-link if found
async function findDriverRecord(profile) {
  // 1. profile_id
  if (profile.id) {
    const { data } = await supabase
      .from('v_driver_summary').select('*')
      .eq('profile_id', profile.id).maybeSingle();
    if (data) { console.log('Driver found by profile_id'); return data; }
  }
  // 2. exact full_name
  if (profile.full_name) {
    ...
    if (data) {
      console.log('Driver found by exact name:', profile.full_name);
      await supabase.from('drivers').update({ profile_id: profile.id }).eq('id', data.id);
      return data;
    }
  }
  // 3. ilike full_name
  if (profile.full_name) {
    ...
    if (data) {
      console.log('Driver found by ilike name:', profile.full_name);
      await supabase.from('drivers').update({ profile_id: profile.id }).eq('id', data.id);
      return data;
    }
  }
  ...
  if (profile.email) {
    ...
    if (data) {
      console.log('Driver found by email:', profile.email);
      await supabase.from('drivers').update({ profile_id: profile.id }).eq('id', data.id);
      return data;
    }
  }
  console.error('No driver record found for:', profile.full_name, profile.email);
  return null;
}
...
export function DriverOverview() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      console.log('Loading driver for profile:', profile.full_name, profile.email);
      const drv = await findDriverRecord(profile);
      ...
      console.log('Payments loaded:', pays?.length);
      setPayments(pays || []);
      setLoading(false);
    }
    load();
  ...
  if (!driver) return (
    <AppLayout>
      ...
      <div style={{ background:C.white, borderRadius:12, padding:32, textAlign:'center', boxShadow:'0 2px 8px rgba(0,0,0,0.07)' }}>
        ...
        <div style={{ color:'#888', fontSize:13 }}>Contact admin to link your account to your driver record.</div>
      </div>
    </AppLayout>
  );
  ...
  const pct       = Number(driver.pct_paid      || 0) * 100;
  ...
  const statusColour = {
    'Completed':'#27AE60','On Track':'#2980B9',
    'In Progress':'#F39C12','At Risk':'#E74C3C',
    ...
    <AppLayout>
      ...
      <div style={{
        ...
        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', flexWrap:'wrap', gap:10, marginBottom:14 }}>
          ...
          <span style={{ background:`${statusColour}22`, color:statusColour, borderRadius:20, padding:'5px 14px', fontWeight:800, fontSize:13 }}>
        ...
        <div style={{ background:'#E8EAED', borderRadius:10, height:isMobile?20:24, overflow:'hidden', marginBottom:8 }}>
          <div style={{
            ...
            justifyContent:'flex-end', paddingRight:8, transition:'width 1.2s ease',
          }}>
      ...
      <div style={{ display:'flex', borderBottom:`2px solid ${C.lgray}`, marginBottom:16 }}>
        {[{id:'charts',label:'Payment Charts'},{id:'history',label:`History (${payments.length})`}].map(t=>(
      ...
      {tab==='charts' ? (
        chartData.length===0
          ? <div style={{ background:C.white, borderRadius:12, padding:32, textAlign:'center', color:'#888' }}>No payment records yet.</div>
...
export function DriverPayments() {
  ...
  return (
    <AppLayout>
      <PageHeader title="My Payment History" subtitle={`Total: GH₵ ${fmt(total)} · ${payments.length} payments`}/>
