...
import { AppLayout, PageHeader, StatCard, KpiRow, ChartCard, C, fmt, Spinner, useIsMobile } from '../components/UI';
    ...
    <AppLayout>
      ...
      <div style={{
        ...
        <ChartCard title="Driver: Vehicle Cost vs Total Paid (GH₵)" height={isMobile ? 220 : 260}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={driverData} margin={{ top:0, right:0, left:0, bottom: isMobile ? 40 : 60 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
              <XAxis dataKey="full_name" tick={{ fontSize: isMobile ? 9 : 10 }} angle={-35} textAnchor="end" interval={0} />
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie
                ...
                cx="50%" cy="50%" outerRadius={isMobile ? 70 : 90}
                label={({ name, percent }) => `${name.split(' ')[0]} ${(percent*100).toFixed(0)}%`}
                labelLine={false} fontSize={isMobile ? 9 : 11}
              >
      ...
      <div style={{
        ...
        <ChartCard title="Investor Payout Progress (GH₵)" height={isMobile ? 220 : 260}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={invData} margin={{ top:0, right:0, left:0, bottom: isMobile ? 40 : 60 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#eee" />
              <XAxis dataKey="full_name" tick={{ fontSize: isMobile ? 9 : 10 }} angle={-35} textAnchor="end" interval={0} />
              <YAxis tick={{ fontSize:10 }} tickFormatter={v => `GH₵${(v/1000).toFixed(0)}k`} width={50} />
              <Tooltip formatter={v => `GH₵ ${fmt(v)}`} />
        ...
        <ChartCard title="Driver Payment Progress (%)" height={isMobile ? 280 : 260}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              ...
              margin={{ top:0, right:30, left: isMobile ? 60 : 80, bottom:0 }}
            >
              <CartesianGrid strokeDasharray="3 3" stroke="#eee" horizontal={false} />
              <XAxis type="number" domain={[0,100]} tick={{ fontSize:10 }} tickFormatter={v => `${v}%`} />
              <YAxis type="category" dataKey="full_name" tick={{ fontSize: isMobile ? 9 : 10 }} width={isMobile ? 58 : 78} />
