...
// Trim leading/trailing spaces from keys ONLY — preserve Date objects intact
...
// ─────────────────────────────────────────────────────────────────────────────
// CONFIRMED EXACT KEYS after cleanRow (verified against the real Excel file):
//
// Investor_Summary:  Full Name, Contact, Email, ID, No. of Vehicles,
//                    Capital Invested, Future Value, Weekly Payout, Weeks Paid,
//                    Total Amount Paid, Balance, Percentage,
//                    No. of Weeks to fully pay, End of Contract Date, Status
//
// Driver_Summary:    Full Name, Contact, email, Driver License,
//                    Vehicle (Make and Model), Vehicle Registration, Investor,
//                    Cost of Vehicle, Weekly Amount, Weeks Paid,
//                    Total Amount Paid, Balance, Percentage, Status
//
// Driver_Payments:   Name, Date, Amt. Paid, Payment Channel, Transaction ID
//
// Investor_Payouts (range:1 — skip merged row 0):
...
//            NOTE: "Amt. Paid _1" has an INTERNAL space — trim() preserves it
// ─────────────────────────────────────────────────────────────────────────────
...
export async function uploadWorkbook(file, onProgress) {
  ...
  // Investor_Payouts: row 0 = merged "Inflow/Outflow" label, row 1 = real headers
  ...
  for (const row of investorRows) {
    ...
    // Profile lookup — no role filter here to avoid RLS recursion issues
  ...
  for (const row of iPayRows) {
    // ── INFLOW (cols A-E) — keys after cleanRow ──
    const iName = txt(row['Name'] || '');
    const iAmt  = num(row['Amt. Paid']);
    const iDate = parseDate(row['Date']);

    if (iName && iAmt > 0) {
      let invId = investorNameToId[iName];
      if (!invId) {
        const f = Object.entries(investorNameToId)
    ...
    // ── OUTFLOW (cols I-M) ──
    // XLSX.js deduplicates repeated headers: 'Amt. Paid ' becomes 'Amt. Paid _1'
    // The internal space between 'Paid' and '_1' is preserved by trim()
    const oName = txt(row['Name_1'] || '');
    const oAmt  = num(row['Amt. Paid _1']);
